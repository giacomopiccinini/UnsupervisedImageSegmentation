...
class SegNet(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.conv_fixed_top = nn.Conv2d(
            self.input_dim, self.n_features, kernel_size=3, stride=1, padding=1
        )
        ...
        for i in range(self.n_convolutions - 1):
            ...
            self.conv_dynamic.append(
                nn.Conv2d(
                    self.n_features, self.n_features, kernel_size=3, stride=1, padding=1
                )
            )
        ...
        self.conv_fixed_bottom = nn.Conv2d(
