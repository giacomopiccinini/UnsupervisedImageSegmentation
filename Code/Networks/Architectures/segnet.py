...
class SegNet(nn.Module):
    ...
    def __init__(self, input_dim):
        ...
        # Write options
        options = load("architecture.yaml")

        self.n_features     = options["n_features"]
        self.n_classes      = options["n_classes"]
        self.n_convolutions = options["n_convolutions"]

        # Add FIXED top layers to architecture:

        # Add convolutional layer
        self.conv_fixed_top = nn.Conv2d(input_dim, self.n_features, kernel_size=3, stride=1, padding=1)
